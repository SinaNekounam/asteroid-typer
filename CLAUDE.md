...
"Star Speller" — a single-file, dependency-free typing game for kids (ages ~1-12). Words ("asteroids") fall
from the top of the screen toward a ship; the player types each word exactly to destroy it before it lands.
Instead of difficulty levels, the player picks a subject **pack** (Animals, Colors, Space, Math, etc.) — 12
free packs and 12 premium (visually locked, no payment integration yet).

Web-only by design — there was a Capacitor-based iOS/Android detour (native projects, CI mobile builds,
Node/npm tooling) that was deliberately reverted; don't reintroduce that unless explicitly asked again.
Everything lives in [index.html](index.html) (HTML + a `<style>` block + a single `<script>` IIFE) — no
framework, no build step, no package manager.

The web build doubles as an installable PWA: `manifest.json` + `service-worker.js` (offline caching) are
linked from `index.html`'s `<head>`. Service worker registration is guarded to skip on plain `http://`
(registration requires a secure context) — see the small `<script>` block right before `</body>`. Icon
files for the PWA live in `icons/` (already generated; there's no source/regeneration tooling committed —
recreate them by hand or with any icon generator if they ever need to change).

## Running it

There's no bundler/dev-server dependency — just serve the static file and open it in a browser:
...
4. **Main loop** (`tick`) — a single `requestAnimationFrame` loop drives spawning and falling. Movement is
   **delta-time normalized**, not frame-count based: `frames = dt / (1000/60)`, then
   ...
   difficulty config; `BASE_FALL_SPEED`/`BASE_SPAWN_MS`/`LIVES` are the same for every pack. The level ramp
   itself (`WORDS_PER_LEVEL`, the `0.08` per-level speed multiplier in `spawnAsteroid`, and the `0.95` spawn-
   interval decay in `maybeLevelUp`) was deliberately softened after user feedback that difficulty spiked too
   fast — if tuning this again, err toward gentler curves, not steeper ones, given the young-kid audience.
